...
# VERSIÓN: 4.1 (CORREGIDO - ORDEN JERÁRQUICO CORRECTO)
# FECHA: Marzo 2026
# =============================================================================
...
# Configuración de la base de datos (PostgreSQL en Supabase o SQLite local como fallback)
...
def login():
    """Muestra el portal de acceso y maneja la autenticación"""
    if 'user' in session:
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = "Error al intentar iniciar sesión. Verifique sus datos."
...
def get_voluntarios():
    ...
    def get_rank_weight(v):
        ...
        # Normalizar: reemplazar tildes y espacios
        rank = rank.replace('PRIMER TENIENTE', 'PRIMER TENIENTE').replace('TENIENTE CORONEL', 'TENIENTE CORONEL')
    ...
    def ordenar_voluntario(v):
        ...
        if rank_name == "DISTINGUIDO":
            return (rank_weight, v.id)  # Solo rango e ID
...
if __name__ == '__main__':
    ...
    print("\n🚀 SERVIDOR INICIADO - Sistema completo con orden jerárquico corregido")
    ...
    print("\n📋 ORDEN JERÁRQUICO CORREGIDO:")
    print("   - PRIMER TENIENTE ahora está entre TENIENTE CORONEL y TENIENTE")
    print("   - DISTINGUIDOS se ordenan por ID (más antiguo primero)")
